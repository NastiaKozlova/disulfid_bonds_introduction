...
df_bonds<-read.csv("min_length.csv",stringsAsFactors = F)
df_bonds<-df_bonds%>%select(bonds)
df_structure<-df_structure[df_structure$bonds%in%df_bonds$bonds,]
...
for (i in 1:nrow(df_structure)) {
  if (file.exists(paste0('protein_SMD/',df_structure$bonds[i],'.pdb'))) {
    ...
    df_tcl[1,1]<-paste0('cd ',part,'stablilisation/','\n',
                        ...
                        'patch DISU U:',df_structure$C3[i],' U:',df_structure$C4[i],'\n',
