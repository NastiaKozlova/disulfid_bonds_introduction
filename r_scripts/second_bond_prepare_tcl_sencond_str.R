...
number_y<-c(0:10000)
setwd(part_start)
df_structure<-read.csv(paste0(part_start,"protein/disulfid_bonds.csv"),stringsAsFactors = F)
...
for (i in 1:length(v_start)) {
  ...
  for (j in 1:nrow(df_structure)) {
      for (q in 1:2) {
        if (file.exists(paste0(v_start[i],"/quench/quench_NaPi2b_",df_structure$bond[j],"_",q,".dcd"))) {
        if (!dir.exists(paste0(v_start[i],"/din/pdb_second/NaPi2b_",df_structure$bond[j],"_",q))) {dir.create(paste0(v_start[i],"/din/pdb_second/NaPi2b_",df_structure$bond[j],"_",q))}
        df_tcl<-data.frame(matrix(nrow = 1,ncol = 7))
        df_tcl[1,1]<-paste0("cd ", part,v_start[i],"\nmol new {protein/NaPi2b_",df_structure$bond[j],".psf} type {psf}")
        df_tcl[1,2]<-paste0("mol addfile {quench/quench_NaPi2b_",df_structure$bond[j],"_",q,".dcd} type {dcd} first 0 last -1 step 1 waitfor all")
        df_tcl[1,3]<-paste0("set nf [molinfo top get numframes]")
        df_tcl[1,4]<-paste0("for {set i 0 } {$i < $nf} {incr i} {")
        df_tcl[1,5]<-paste0('[atomselect top "resname CYS" frame $i] writepdb din/pdb_second/NaPi2b_',df_structure$bond[j],'_',q,'/frame_$i.pdb')
        df_tcl[1,6]<-paste0("}")
        df_tcl[1,7]<-"mol delete all"
        df_tcl[1,8]<-"exit now"
        write.table(df_tcl,paste0(v_start[i],"/din/tcl/Second_str_NaPi2b_",df_structure$bond[j],"_",q,".tcl"),sep="\n",col.names = F,row.names = F,quote = F)
        
        system(command = paste0("vmd -dispdev text -e ", v_start[i],"/din/tcl/Second_str_NaPi2b_",df_structure$bond[j],"_",q,".tcl"),ignore.stdout=T,wait = T)
      }
    }
  }
}
