...
#  l<-(-1)
  ...
  for (j in 1:nrow(df_structure)) {
    ...
    pdb<-read.pdb(paste0("protein/",df_structure$first_bond[j],".pdb"))
    df_pdb<-pdb$atom
    for (q in 1:2) {
      if (q==1){w<-2}else{w<-1}
      df_pdb_C<-df_pdb%>%filter(elety=="SG")  
      df_pdb_C3<-df_pdb_C%>%filter(resno==df_structure[j,(q+3)])
      df_pdb_C4<-df_pdb_C%>%filter(resno==df_structure[j,(w+3)])
      len<-sqrt((df_pdb_C3$x[1]-df_pdb_C4$x[1])^2+(df_pdb_C3$y[1]-df_pdb_C4$y[1])^2+(df_pdb_C3$z[1]-df_pdb_C4$z[1])^2)
      x<-(df_pdb_C3$x[1]-df_pdb_C4$x[1])/len
      y<-(df_pdb_C3$y[1]-df_pdb_C4$y[1])/len
      z<-(df_pdb_C3$z[1]-df_pdb_C4$z[1])/len
