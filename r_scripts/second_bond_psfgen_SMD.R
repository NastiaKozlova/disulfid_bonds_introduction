...
for (i in 1:length(v_start)) {
  ...
  for (j in 1:nrow(df_structure)) {
    if (file.exists(paste0('protein/',df_structure$first_bond[j],'.pdb'))) {
      fixed_1<-(df_structure$C3[j]-10):(df_structure$C3[j]+10)
      fixed_2<-(df_structure$C4[j]-10):(df_structure$C4[j]+10)
      if(min(fixed_2)-max(fixed_1)<10){
        fixed_1<-(df_structure$C3[j]-10):(df_structure$C4[j]-10)
        fixed_2<-(df_structure$C3[j]+10):(df_structure$C4[j]+10)
      }
      ...
      df_tcl[9,1]<-paste0("segment U { pdb protein/",df_structure$first_bond[i] ,".pdb")
      df_tcl[10,1]<-paste0('}\n',
                           "patch DISU U:",df_structure$C1[i]," U:",df_structure$C2[i])
      
      df_tcl[11,1]<-paste0('coordpdb protein/',df_structure$first_bond[i] ,'.pdb U')
      ...
      df_tcl[9,1]<-paste0('set fixed [atomselect top " protein and (resid  ',df_structure$C3[j],')"]')
      ...
      df_tcl[5,1]<-paste0('set all [atomselect top "all"]')
      df_tcl[6,1]<-paste0('$all set beta 1')
      #      df_tcl[7,1]<-paste0('set unfixed [atomselect top " protein and ((resid>',(df_structure$C4[j]-10),' and resid<',(df_structure$C4[j]+10),') or ',
      #                          '(resid>',(df_structure$C3[j]-10),' and resid<',(df_structure$C3[j]+10),'))"]')     
      df_tcl[7,1]<-paste0('set unfixed [atomselect top " protein and (resid ',paste0(fixed,collapse = " "),')"]') 
      df_tcl[8,1]<-paste0('$unfixed set beta 0')
      df_tcl[9,1]<-paste0('set fixed [atomselect top " protein and (resid  ',df_structure$C4[j],')"]')
      df_tcl[10,1]<-paste0('$fixed set beta 1')
      ...
      df_tcl[7,1]<-paste0('set fixed [atomselect top " protein and resid  ',df_structure$C4[j],'"]')
      ...
      df_tcl[5,1]<-paste0('set all [atomselect top "all"]')
      df_tcl[6,1]<-paste0('$all set occupancy 0')
      df_tcl[7,1]<-paste0('set fixed [atomselect top " protein and resid  ',df_structure$C3[j],'"]')
      df_tcl[8,1]<-paste0('$fixed set occupancy 1')
